...
class DataTracker {
  ...
  installHooks(mod) {
    ...
    this.installHook(mod, "S_USER_LEVELUP", 2, (event) => {
      ...
      if (this.SpawnedPlayers[cid] != null) this.SpawnedPlayers[cid]["level"] = event.level
    })

    this.installHook(mod, "S_PLAYER_STAT_UPDATE", 14, (event) => {
      this.MyStats = event
    })
    ...
    this.installHook(mod, "S_PLAYER_CHANGE_MP", 1, (event) => {
      let cid = event.target.toString()
      if (cid == this.MyID && this.MyStats) this.MyStats.mp = event.currentMp
    })

    this.installHook(mod, "S_PLAYER_CHANGE_STAMINA", 1, (event) => {
      if (this.MyStats) this.MyStats.stamina = event.current
    })

    mod.game.inventory.on("update", () => {
      this.MyEquipment = {}
      for (let item of mod.game.inventory.equipmentItems) {
        let passivities = item.mergedPassivities
        let masterworkBonus = 0

        let activePassivities = item.passivitySets[item.passivitySet]
        if (activePassivities) {
          passivities = passivities.concat(activePassivities.passivities.filter((passivity) => passivity !== 0))
    ...
    this.installHook(mod, "S_CREST_INFO", 2, (event) => {
      this.MyGlyphs = {}
      for (let glyph of event.crests) this.MyGlyphs[glyph.id] = glyph.enable
    })
    ...
    this.installHook(mod, "S_NPC_LOCATION", 3, (event) => {
      let cid = event.gameId.toString()
      if (this.SpawnedNPCs[cid]) this.SpawnedNPCs[cid]["location"] = [event.loc.x, event.loc.y, event.loc.z, event.w]
    })

    this.installHook(mod, "S_GROUP_NPC_LOCATION", 1, (event) => {
      for (let npc of event.npcs) {
        let cid = npc.gameId.toString()
        if (this.SpawnedNPCs[cid]) this.SpawnedNPCs[cid]["location"] = [npc.loc.x, npc.loc.y, npc.loc.z, npc.w]
      }
    })

    this.installHook(mod, "S_CREATURE_ROTATE", 2, (event) => {
      let cid = event.gameId.toString()
      if (this.SpawnedNPCs[cid]) this.SpawnedNPCs[cid]["location"][3] = event.w
    })

    this.installHook(mod, "S_INSTANT_MOVE", 3, (event) => {
      let cid = event.gameId.toString()
      if (cid == this.MyID) this.MyLocation = [event.loc.x, event.loc.y, event.loc.z, event.w]
      else if (this.SpawnedNPCs[cid]) this.SpawnedNPCs[cid]["location"] = [event.loc.x, event.loc.y, event.loc.z, event.w]
    })
    ...
    this.installHook(mod, "S_NPC_OCCUPIER_INFO", 1, (event) => {
      ...
      if (this.SpawnedPlayers[event.pid.toString()] && this.SpawnedPlayers[event.pid.toString()]["level"] > npc["occupierLevel"]) npc["occupierLevel"] = this.SpawnedPlayers[event.pid.toString()]["level"]

      if (this.SpawnedPlayers[event.cid.toString()] && this.SpawnedPlayers[event.cid.toString()]["level"] > npc["occupierLevel"]) npc["occupierLevel"] = this.SpawnedPlayers[event.cid.toString()]["level"]
    })
    ...
    this.installHook(mod, "S_SPAWN_USER", 17, (event) => {
      let cid = event.gameId.toString()
    ...
    this.installHook(mod, "S_ABNORMALITY_BEGIN", 3, (event) => {
      ...
      if (this.SpawnedPlayers[cid]) this.SpawnedPlayers[cid]["abnormalities"][event.id] = abnormality
      else if (this.SpawnedNPCs[cid]) this.SpawnedNPCs[cid]["abnormalities"][event.id] = abnormality
    })
    ...
    this.installHook(mod, "S_ABNORMALITY_END", 1, (event) => {
      ...
      if (this.SpawnedPlayers[cid]) delete this.SpawnedPlayers[cid]["abnormalities"][event.id]
      else if (this.SpawnedNPCs[cid]) delete this.SpawnedNPCs[cid]["abnormalities"][event.id]
    })

    this.installHook(mod, "S_SKILL_LIST", 2, (event) => {
      this.MyPassivities = []
      for (let skill of event.skills) {
        if (!skill.active) this.MyPassivities.push(Number(skill.id & (mod.majorPatchVersion < 74 ? 0xffffffff : 0xffffffffn)) | 0)
      }
    })
    ...
    this.installHook(mod, "S_GET_USER_LIST", 20, (event) => {
      this.MyCaractersName = {
        Id: event.id,
        Name: event.name, //player name
        GuildName: event.guildName, //player guild name
        Level: event.level, // player lvl
        LastLogoutTime: event.lastLogoutTime, // player last connection
        AdventureCoins: event.adventureCoins, //player aventure coins remaining
        HasBrokerSales: event.hasBrokerSales, // player has dell in broker
        PetAdventureStatus: event.petAdventureStatus, // player partener aventures # 0: inactive, 1: active, 2: finished
        DeleteRemainSec: event.deleteRemainSec,
      }
    })
  }
